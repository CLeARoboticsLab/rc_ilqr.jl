...
function solve_ilqr(dynamics_function :: Function, cost_function :: Function, Q :: Matrix{Float64},
    ...
    # backwards pass
        for k = T - 1 : -1 : 1
            ...
            delta_u[k] = (δx) -> -K * δx - Kᵥ * v_all[k + 1] - Kᵤ * nominal_control_sequence[k]
        end
        ...
        for k = 2 : T
            new_nominal_state_sequence[k] = dynamics_function(
                ...
                nominal_control_sequence[k - 1] + delta_u[k - 1](new_nominal_state_sequence[k - 1] - nominal_state_sequence[k - 1]))
        end

        new_cost = cost_function(Q, R, Q_T, new_nominal_state_sequence, [nominal_control_sequence[i] + delta_u[i](nominal_state_sequence[i] - nominal_state_sequence[i]) for i in 1:T - 1], x_T, T)
        ...
        elseif delta_cost <= threshold
            ...
            println("num iters: ", iter)
            return [new_nominal_state_sequence, [nominal_control_sequence[i] + delta_u[i](nominal_state_sequence[i] - nominal_state_sequence[i]) for i in 1:T - 1]]
        else
            # println("")
            nominal_state_sequence = new_nominal_state_sequence
            # nominal_control_sequence += step_size * delta_u
            nominal_control_sequence = [nominal_control_sequence[i] + delta_u[i](nominal_state_sequence[i] - nominal_state_sequence[i]) for i in 1:T - 1]
            old_cost = new_cost
        end
        iter += 1
    end
...
function solve_ilqr_v2(dynamics_function, cost_function, Q, Q_T, R, x_T, xₒ,
    ...
    delta_u = Array{Function}(undef,1,T - 1)
    ...
    # backwards pass
        ...
        for k = 2 : T
            new_nominal_state_sequence[k] = dynamics_function(
                new_nominal_state_sequence[k - 1],
                # nominal_control_sequence[k - 1] + delta_u[k - 1](delta_x[k - 1]))
                nominal_control_sequence[k - 1] + α * delta_u[k - 1](new_nominal_state_sequence[k - 1] - nominal_state_sequence[k - 1]))
            
            # delta_x[k] = A_call(nominal_state_sequence[k-1],nominal_control_sequence[k-1]) * delta_x[k-1] + B_call(nominal_state_sequence[k-1],nominal_control_sequence[k-1]) * delta_u[k-1](delta_x[k-1])
        end
        println("---new iter info-----")
        # Main.@infiltrate
        # xs = [p[1] for p in vec(new_nominal_state_sequence)]
        # ys = [p[2] for p in vec(new_nominal_state_sequence)]
        # scatterlines!(ax, [Point(x, y) for (x, y) in  zip(xs,ys)])


        temp = [nominal_control_sequence[i] + α * delta_u[i](new_nominal_state_sequence[i] - nominal_state_sequence[i]) for i in 1:T - 1]
        new_cost = cost_function(Q, R, Q_T, new_nominal_state_sequence, temp, x_T, T)
        ...
        if delta_cost < 0
            if α <= 0.0001
                ...
                println("ilqr step increased cost")
                println("num iters: ", iter)
                println("new stuff: ", new_nominal_state_sequence)
                println("---------")
                println(temp)
                println("-------------------- returning out of solve --------------") 
                fig
                return [nominal_state_sequence, nominal_control_sequence]
            else
                α = α / 10
        ...
        elseif delta_cost <= threshold
            ...
            println("")
            fig
            return [new_nominal_state_sequence, temp]
        else
            # println("")
            old_cost = new_cost
            nominal_state_sequence = new_nominal_state_sequence
            nominal_control_sequence = temp
        end
        iter += 1
...
export solve_ilqr, solve_ilqr_v2
#  solve_ilqr_v3